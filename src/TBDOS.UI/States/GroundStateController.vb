...
Friend Class GroundStateController
    ...
    Private Sub TakeItems(amount As Integer)
        _world.Avatar.Location.Inventory.Stack(_itemType).Remove(amount)
    ...
    Private ReadOnly Property ItemCount As Integer
        Get
            If _itemType IsNot Nothing Then
                Return _world.Avatar.Location.Inventory.Stack(_itemType).Quantity
            End If
            Return 0
        End Get
    End Property
    ...
    Private Function UpdateGeneral() As UIStates
        If _world.Avatar.Location.Inventory.Stacks.Any Then
            ...
            _screen.WriteLine(String.Join(", ", _world.Avatar.Location.Inventory.Stacks.Select(Function(x) $"{_world.InventoryName(x.InventoryName)}(x{x.Quantity})")))
            _screen.WriteLine("[esc] Go Back")
            Return UIStates.Ground
        Else
            Return UIStates.InPlay
